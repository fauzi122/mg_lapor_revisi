        ...
        <div class="app-toolbar-wrapper d-flex flex-stack flex-wrap gap-4 w-100">
            ...
            <div class="d-flex align-items-center gap-2 gap-lg-3">
                <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0">
                    ...
                    <li class="breadcrumb-item text-muted">Impor</li>
                    <li class="breadcrumb-item">
                        <span class="bullet bg-gray-400 w-5px h-2px"></span>
                    </li>
                    ...
                    <div class="card">
                        <div class="card-header align-items-center px-2">
                            ...
                            <div class="card-title flex-row-fluid justify-content-end gap-5">
                                <input type="hidden" class="export-title" value="Laporan Impor {{ $per->nama_perusahaan }}" />
                            </div>
                        </div>
                        <table class="kt-datatable table table-bordered table-hover">
                            ...
                            <tbody class="fw-semibold text-gray-600">
                                @foreach ($query as $data)
                                    ...
                                    @endphp --}}
                                    @php
                                        $kodeBulan = Crypt::encryptString('bulan,' . $data->bulan_pib . ',' . $data->npwp);
                                        $kodeTahun = Crypt::encryptString('tahun,' . $data->bulan_pib . ',' . $data->npwp);
                                    @endphp
                                    <tr>
                                        <td>{{ $loop->iteration }}</td>
                                        <td>
                                            <b><a href="{{ url('/laporan/impor/exim') }}/{{ $kodeBulan }}">
                                                {{ getBulan($data->bulan_pib) }}
                                                <i class="bi bi-check" title="Lihat laporan per bulan"></i>
                                            </a></b>
                                        </td>
                                        <td>
                                            <b><a href="{{ url('/laporan/impor/exim') }}/{{ $kodeTahun }}">
                                        ...
                                        @if ($data->status == 1)
                                            <td class="text-center">
                                                <button type="button"
                                                    ...
                                                    data-bs-target="#kt_modal_update_{{ getBulan($data->bulan_pib) }}" title="Revisi data">
                                                ...
                                                @if ($data->status == 1 && $data->catatan)
                                                    <button
                                                        ...
                                                        data-b="{{ \Illuminate\Support\Facades\Crypt::encrypt($data->bulan_pib) }}"><i
                                                            class="bi bi-check"
                                                            title="Selesai"></i></button>
                                                @endif

                                                <div class="modal fade" id="kt_modal_update_{{ getBulan($data->bulan_pib) }}" tabindex="-1" aria-hidden="true">
                                                    <!--begin::Modal dialog-->
                                                ...
                                                <tbody>
                                                    @foreach ($query as $data)
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td>
                                                                <b><a
                                                                        href="{{ url('/laporan/impor/exim') }}/{{ $id }}">{{ getBulan($data->bulan_pib) }}
                                                                        <i class="bx bx-check"
                                                                            title="lihat data laporan"></i></a><b>
                                                            </td>
                                                            <td>
                                                                <b><a
                                                                        href="{{ url('/laporan/impor/exim') }}/{{ $idTahun }}">{{ getTahun($data->bulan_pib) }}
                                                                        <i class="bx bx-check"
                                                                            title="lihat data laporan"></i></a><b>
                                                            </td>
                                                            <td>
                                                                    ...
                                                                    <div class="modal fade" id="modal-update"
                                                                        ...
                                                                        <div class="modal-dialog modal-lg">
                                                                            <div class="modal-content">
                                                                                ...
                                                                                <form
                                                                                    action="{{ url('/laporan/impor/exim/update-revision-all') }}"
                                                                                    method="post" id="updateStatusForm"
                                                                                    enctype="multipart/form-data">
                                                                                    @csrf
                                                                                    <input type="hidden" name="p"
