                            ...
                            <div class="row w-100">
                                ...
                                <div class="col-lg-6">
                                    <div class="d-flex justify-content-end gap-2">
                                        ...
                                        @elseif ($statusx == 2)
                                            <form action="{{ url('/submit_bulan_pggb') }}/{{ $id }}"
                                        ...
                                        @else
                                            ...
                                            <button type="button" class="btn btn-sm btn-primary"
                                                onclick="produk(); provinsi(); kab_kota(); tambahPMB('{{ $bulan_ambilx }}' )"
                                                data-bs-toggle="modal" data-bs-target="#myModal">
                                                <i class="fas fa-plus"></i> Buat Laporan {{ dateIndonesia($bulan_ambilx) }}
                                            </button>
                                            <button type="button" class="btn btn-sm btn-success"
                                                onclick="tambahPMB('{{ $bulan_ambilx }}' )" data-bs-toggle="modal"
                                                data-bs-target="#excelpggb">
                                                <i class="fas fa-upload"></i> Import Excel
                                            </button>
                                        @endif
                                    </div>
                                        ...
                                        <tbody>
                                            @foreach ($pggb as $pggb)
                                                <tr>
                                                    ...
                                                    <td>{{ $pggb->tanggal_awal }} s.d. {{ $pggb->tanggal_berakhir }}</td>
                                                    <td>{{ $pggb->tarif_penyimpanan }}</td>
                                                    <td>{{ $pggb->satuan_tarif }}</td>
                                                    <td class="text-center">
                                                        @if ($pggb->status == '0')
                                                            <button type="button" class="btn btn-sm btn-info editpggb mb-2"
                                                                id="editCompany"
                                                            ...
                                                            <form action="{{ url('/hapus_pggb') }}/{{ $pggb->id }}"
                                                                method="post" class="d-inline">
                                                                @method('delete')
                                                                @csrf
                                                        ...
                                                        @elseif($pggb->status == '2')
                                                            <button type="button"
                                                                class="btn btn-sm btn-info editpggb mb-2" id="editCompany"
                                                                onclick="editpggb('{{ $pggb->id }}', '{{ $pggb->kab_kota }}' , '{{ $pggb->produk }}' )"
                                                                data-bs-toggle="modal" data-bs-target="#modal-edit"
                                                                data-id="{{ $pggb->id }}">
                                                                <i class="ki-solid ki-pencil" title="Edit Data"></i>
